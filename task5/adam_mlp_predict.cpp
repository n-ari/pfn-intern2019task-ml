...
int main(){
    ...
    for(int tries=1; tries<=TRY_NUMS; tries++){
        ...
        std::cerr << tries << "th try: ";
        std::cerr << "loss=" << std::fixed << std::setprecision(5) << averageLoss << ", ";
        std::cerr << "rate=" << std::fixed << std::setprecision(5) << averageRate << std::endl;
        if(averageRate > bestRate || (averageRate == bestRate && averageLoss < bestLoss)){
    ...
    // best model
    std::cerr << "best loss=" << std::fixed << std::setprecision(5) << bestLoss << ", ";
    std::cerr << "best rate=" << std::fixed << std::setprecision(5) << bestRate << std::endl;

    // output prediction
    std::ofstream ofs("../../prediction_mlp_relu_sigmoid.txt");
    const int TRUE_TEST_NUMS = 500;
    for(int i=0; i<TRUE_TEST_NUMS; i++){
        graph G = loadGraphFromFile("../../datasets/test/" + std::to_string(i) + "_graph.txt");
        bool label = calcLabel(G,T,bestTheta);
        ofs << label << std::endl;
    }
    ofs.close();
	return 0;
